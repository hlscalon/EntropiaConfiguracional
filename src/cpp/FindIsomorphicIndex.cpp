...
int find_isomorphic_index(const Vector<unsigned long*> & ngraphs, const graph * clgraph, int totalNodes) {
	auto itIdx = __gnu_parallel::find_if(ngraphs.begin(), ngraphs.end(), [&clgraph](const graph * clg) {
		return is_isomorphic(clg, clgraph, totalNodes);
	});

	return itIdx == ngraphs.end() ? -1 : itIdx - ngraphs.end();
}
